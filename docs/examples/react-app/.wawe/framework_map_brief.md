...
- npm scripts: `dev` → vite, `test` → vitest

## Public surface of the code

- `src/api.ts`: BASE_URL, getUser
- `src/App.tsx`: App
...
- src/api.ts (0 KB), src/App.tsx (0 KB)
...
- product exposes 1: root … (full list in the full map)
...
## Which component owns which test id (1)

- `root` — App.tsx

## Test configuration

- `package.json`: {"name":"app","scripts":{"dev":"vite","test":"vitest"}}
