...
| `xrefs` | [{subject, edge, object, file, candidates, line, resolution}] | every edge of the graph as a row, with the rule that placed it, sorted by subject, object and line and capped by nothing. `edge` is `calls`, `declares` or `imports`. A `calls` row is one cross-file call: `subject` is `<file>:<func>` with the file relative to the repository, `object` the callee's name, `candidates` every indexed file that declares it as the edge names them, `file` the one of them the edge settled on and null where it named several, and `line` the first line of the subject the callee is called on. `call_graph_files` above is rendered from these rows, so the two never disagree; the rendering is cut to 8 callees a key and 60 keys and these rows are not. A `declares` row is a `spans` site: `subject` and `file` are the declaring file, spelled as `spans` and `definitions` spell it, absolute, and `line` is where the declaration starts. An `imports` row is one `import_graph` dependency, package on package: no file, no candidates and a null `line`, because that graph is every import line of a package read together. `resolution` is one of seven values, each written by the rule that placed the edge and by nothing else: `sole_declarer`, one indexed file declares the name; `import_line`, the caller's own `from M import name` line (or an `import {name} from "./a"` in TypeScript or JavaScript) named the file, and on an `imports` row the import lines the package graph was read from; `receiver_import`, the module the receiver of a `NAME.attr(...)` call was bound to named it; `re_export`, the receiver is a facade and its own import lines were followed to the file with the `def`; `base_class`, a `self.name(...)` or `cls.name(...)` call reached the one base class, in another file, that declares it; `declaration`, the row is the declaration site itself; and `ambiguous`, several files declare the name and nothing said which, which is the `?` the rendering carries. There is no `local` value and no `overrides` edge: a call inside the file that declares the callee is not written to the graph at all, and no rule computes an override |
