...
| `--diff` | What changed since the map already in `--out`, naming the files whose content hash moved before the map keys that moved with them | the pointer names what moved since the last session; CI step "pointer says what changed since the last session" proves it | — |
...
--diff                       what changed since the map already in --out:
                             the files whose content moved, then the keys
