...
| Every edge as a row that says how it was resolved (`xrefs`) | The graph normalised: one row per edge, `{subject, edge, object, file, candidates, line, resolution}`, sorted by subject, object and line and capped by nothing. `edge` is `calls`, `declares` (a `spans` site) or `imports` (an `import_graph` dependency). `resolution` names the rule that placed the edge and is one of seven values: `sole_declarer` one indexed file declares the name, `import_line` the caller's own import line named the file, `receiver_import` the module the receiver was bound to named it, `re_export` a facade's own import lines were followed to the file with the `def`, `base_class` a `self.name(...)` call reached the one base that declares it, `declaration` the row is the declaration site, and `ambiguous` several files declare the name and nothing said which, which is the `?` the rendering carries. `call_graph_files` is rendered from these rows, so a consumer reading the table and a reader reading the string are reading one fact. There is no `local` value and no `overrides` edge: a call inside the file that declares the callee never becomes an edge, and no rule computes an override | Measured 2026-09-07 on this repository's own map: 868 rows, 200 of them calls (172 `sole_declarer`, 11 `re_export`, 7 `receiver_import`, 5 `import_line`, 5 `ambiguous`) and 668 declarations; `call_graph_stats` counts 195 edges and 5 marked, which is the same 200 rows counted the other way. The map grows from 2,251,198 to 2,598,532 bytes, 15 percent, and the build takes the same 3.3 seconds | Read `xrefs` instead of parsing the pipes and the `?` back out of a rendered edge |
